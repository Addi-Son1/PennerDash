...
async function refreshLeaderboard() {
  try {
    ...
    // V7: Sichtbare Werte nach Reset-Flag berechnen
    const mappedEntries = entries.map((entry) => {
      const hasReset = !!entry.resetV7Applied;
      const baseMoney = typeof entry.money === "number" ? entry.money : (entry.totalMoneyEarned || 0);
      return {
        ...entry,
        visibleLevel: hasReset ? (entry.level ?? 0) : 0,
        visibleTotalBottles: hasReset ? (entry.totalBottles || 0) : 0,
        visibleMoney: hasReset ? baseMoney : 0,
      };
    });

    const sortedEntries = [...mappedEntries].sort((a, b) => {
      if (lbSort === 'bottles') {
        return (b.visibleTotalBottles || 0) - (a.visibleTotalBottles || 0);
      }
      if (lbSort === 'money') {
        return (b.visibleMoney || 0) - (a.visibleMoney || 0);
      }
      // default: sort by level
      return (b.visibleLevel || 0) - (a.visibleLevel || 0);
    });
...
let playersHtml =
    ...
    sortedEntries.slice(0, 100).forEach((entry, idx) => {
      ...
      const moneyValue = entry.visibleMoney ?? 0;
      ...
      playersHtml += `<div class="leaderboard-row ${rowClass}">
        ...
        <div>${entry.visibleLevel ?? 0}</div>
        <div>${entry.visibleTotalBottles ?? 0}</div>
